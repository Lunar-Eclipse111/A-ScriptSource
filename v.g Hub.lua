...
OldNameCall = hookmetamethod(game, "__namecall", function(self, ...)
    local Args = {...}
    local Self = Args[1]
        if Args[4] == "throw" and tostring(self) == "RemoteEvent" and getnamecallmethod() == "FireServer" then
            if Args[6] == "HumanoidRootPart" then
                 Args[7] = Vector3.new(2, 2, 1)
            end
        end
    return OldNameCall(self, unpack(Args))
end)

local Name = "Assassin.txt"
...
local Toggle1 = Section1:CreateToggle("Hitbox Extender", Settings.HitBox, function(State)
                    ...
                    pcall(
                        function()
                            ...
                            game.Workspace[v.Name].HumanoidRootPart.Size =
                                Vector3.new(Settings.Size, Settings.Size, Settings.Size)
                            game.Workspace[v.Name].HumanoidRootPart.Size =
                                Vector3.new(Settings.Size, Settings.Size, Settings.Size)
                        end
                    )
...
local Button1 = Section1:CreateButton("Claim EndOfObby Knife", function()
pcall(function()
firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, game:GetService("Workspace").Lobby.ObbyEnd, 0)
end) end)

local Button1 = Section1:CreateButton("remove VIPBarrier", function()
pcall(function()
game:GetService("Workspace").Lobby.VIPBarrier:remove()
end) end)
